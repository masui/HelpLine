var data = 
{
  "codes": [],
  "defs": [
    "$ ファイルを新しい順に(リスト|表示)する {8}",
    "$ ファイルを新しさ順に(リスト|表示)する {8}",
    "$ 新しいファイルから順番に(リスト|表示)する {8}",
    "% git ls-files | xargs ls -l -t {8}",
    "$ ファイルを大きい(順に|順番で)表示する {9}",
    "$ ファイルを大きさの順に表示する {9}",
    "% git ls-files | xargs du -s | sort -r -n {9}",
    "$ Compare file xxxx with the one from 2 days ago {10}",
    "$ xxxxというファイルを2日前のものと比較する {10}",
    "$ 2日前のxxxxというファイルを現在のものと比較する {10}",
    "% git diff HEAD '@{2 days ago}' xxxx {10}",
    "$ Compare (#{files}) with the one from (#{numbers}) minutes ago {10}",
    "$ 「(#{files})」ファイルを(#{numbers})分前の(もの|バージョン)と比較する {10}",
    "$ 「(#{files})」ファイルが(#{numbers})分前から(変化した|変わった)ところを(#{display}) {10}",
    "% git diff HEAD '@{#{$2} minutes ago}' #{$1} {10}",
    "$ (#{numbers})分前の「(#{files})」ファイルと現在の(もの|バージョン)を比較する {10}",
    "% git diff HEAD '@{#{$1} minutes ago}' #{$2} {10}",
    "$ (#{numbers})日前の「(#{files})」ファイルと現在のものを(比較する|比べる) {10}",
    "% git diff HEAD '@{#{$1} days ago}' #{$2} {10}",
    "$ 現在の「(#{files})」ファイルを(#{numbers})日前のものを(比較する|比べる) {10}",
    "% git diff HEAD '@{#{$2} days ago}' #{$1} {10}",
    "$ (1つ|ひとつ)前のバージョンの(#{files})と比較 {10}",
    "$ (1つ|ひとつ)前のバージョンの(#{files})からの変更点 {10}",
    "% git diff $(git rev-list -n 1 HEAD -- #{$2})^ -- #{$2} {10}",
    "$ (#{files})を(1つ|ひとつ)前のバージョンと比較 {10}",
    "$ (#{files})の(1つ|ひとつ)前のバージョンからの変更点 {10}",
    "% git diff $(git rev-list -n 1 HEAD -- #{$1})^ -- #{$1} {10}",
    "$ (#{files})の最新版の変更点を(#{display}) {10}",
    "$ 最新の(#{files})の変更個所は? {10}",
    "$ (#{files})は最後にどこを変えた? {10}",
    "$ (#{files})の(一番新しい|最新の)(変更|更新)は? {10}",
    "% git diff $(git rev-list -n 1 HEAD -- #{$1})^ -- #{$1} {10}",
    "$ (2つ|ふたつ)前のバージョンの(#{files})と比較 {10}",
    "$ (2つ|ふたつ)前のバージョンの(#{files})からの変更点を(#{display}) {10}",
    "% git diff $(git rev-list -n 1 HEAD -- #{$2})^^ -- #{$2} {10}",
    "$ (#{files})を(2つ|ふたつ)前のバージョンと比較 {10}",
    "$ (#{files})の(2つ|ふたつ)前のバージョンのからの変更点を(#{display}) {10}",
    "% git diff $(git rev-list -n 1 HEAD -- #{$1})^^ -- #{$1} {10}",
    "$ ファイル「(#{files})」の編集履歴を完全に(#{delete})がワーキングツリーは残す {12}",
    "% git filter-branch -f --index-filter 'git rm --cached --ignore-unmatch #{$1}' HEAD {12}",
    "$ ファイル「(#{files})」の編集履歴を完全に(#{delete}) {12}",
    "% git filter-branch -f --index-filter 'git rm --ignore-unmatch #{$1}' HEAD {12}",
    "$ 編集履歴をGitHubに反映させる {12}",
    "% git push origin --force --all {12}",
    "$ ファイル「(#{files})」の(#{numbers})分前からの(#{change})履歴を(#{display}) {13}",
    "% git log --since \"#{$2} minutes ago\" #{$1} {13}",
    "$ (#{numbers})分前からのファイル「(#{files})」の(#{change})履歴を(#{display}) {13}",
    "% git log --since \"#{$1} minutes ago\" #{$2} {13}",
    "$ ファイル「(#{files})」の(#{numbers})分前からの(#{change})を(#{display}) {13}",
    "% git diff HEAD \"@{#{$2} minutes ago}\" #{$1} {13}",
    "$ (#{numbers})分前からのファイル「(#{files})」の(#{change})を(#{display}) {13}",
    "% git diff HEAD \"@{#{$1} minutes ago}\" #{$2} {13}",
    "$ ファイル「(#{files})」の(#{numbers})時間前からの(#{change})履歴を(#{display}) {13}",
    "% git log --since \"#{$2} hours ago\" #{$1} {13}",
    "$ (#{numbers})時間前からのファイル「(#{files})」の(#{change})履歴を(#{display}) {13}",
    "% git log --since \"#{$1} hours ago\" #{$2} {13}",
    "$ ファイル「(#{files})」の(#{numbers})時間前からの(#{change})を(#{display}) {13}",
    "% git diff HEAD \"@{#{$2} hours ago}\" #{$1} {13}",
    "$ (#{numbers})時間前からのファイル「(#{files})」の(#{change})を(#{display}) {13}",
    "% git diff HEAD \"@{#{$1} hours ago}\" #{$2} {13}",
    "$ ファイル「(#{files})」の(#{numbers})日前からの(#{change})履歴を(#{display}) {13}",
    "% git log --since \"#{$2} days ago\" #{$1} {13}",
    "$ (#{numbers})日前からのファイル「(#{files})」の(#{change})履歴を(#{display}) {13}",
    "% git log --since \"#{$1} days ago\" #{$2} {13}",
    "$ ファイル「(#{files})」の(#{numbers})日前からの(#{change})を(#{display}) {13}",
    "% git diff HEAD \"@{#{$2} days ago}\" #{$1} {13}",
    "$ (#{numbers})日前からのファイル「(#{files})」の(#{change})を(#{display}) {13}",
    "% git diff HEAD \"@{#{$1} days ago}\" #{$2} {13}",
    "$ ブランチのリストを更新日時順に表示する {14}",
    "% git branch --sort=committerdate {14}",
    "$ 直前のコミットを取り消す {16}",
