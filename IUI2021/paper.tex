...
\item \textbf{Document expansion by users}
  ...
  Providing entries for {\ES} can solve the problem, but
...
We created the GitHelp system using {\ES}, so that git users can easily
remember command options and use it without perfectly remembering the git commands.


We implemented the HelpLine system
...
If HelpLine is installed,
the user can type a special shortcut key
(e.g. \texttt{Ctrl-J})
to invoke HelpLine after typing \sqtt{git RE 2} to see how he can
perform the task.
...
OmniSearch is an implementation of ExpandSearch that runs on Chrome browser.
...
Figure xxx shows an example {\HF} page that supports finding FAQs
...
A flexible search mechanism ``ExpandSearch'' is used in HelpLine.
%
The database used in ExpandSearch are provided as a set of data entries
